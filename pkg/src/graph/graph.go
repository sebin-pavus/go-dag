...
func (g Graph) TopologicalSort() Stack {
	s := make(Stack, 0)
	visited := map[Node]bool{}
	nodes := g.Nodes()
	for _, node := range nodes {
		if !visited[node] {
			s = g.TopologicalSortUtil(node, visited, s)
		}
	}
	return s
}

func (g Graph) TopologicalSortUtil(node Node, visited map[Node]bool, s Stack) Stack {
	visited[node] = true
	adjacenyList := g.adjacenyList[node]
	for _, edge := range adjacenyList {
		if !visited[edge] {
			s = g.TopologicalSortUtil(edge, visited, s)
		}
	}
	return s.Push(node)
}
